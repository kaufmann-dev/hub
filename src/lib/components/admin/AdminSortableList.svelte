<script lang="ts">
	...
	let flipDurationMs = $derived(prefersReducedMotion.current ? 0 : 160);
...
<div class="relative">
	<ol
		class={[
			'list-none space-y-3 rounded-lg transition-[background-color,box-shadow] duration-150',
	...
	>
		{#each items as item (item.id)}
			<li
				class={[
					'bg-card relative flex items-center gap-3 rounded-lg border py-3 pr-3 pl-12 shadow-xs transition-[border-color,background-color,box-shadow,opacity] duration-150',
			...
			>
				<div
					class={[
						'admin-sort-handle text-muted-foreground hover:bg-accent hover:text-accent-foreground focus-visible:ring-ring absolute inset-y-0 left-0 flex w-12 touch-none items-center justify-center rounded-l-lg transition-colors outline-none focus-visible:z-10 focus-visible:ring-2 focus-visible:ring-inset',
