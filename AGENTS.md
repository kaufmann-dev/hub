...
- **Database**: Use the manually managed Podman PostgreSQL container `postgres-sveltekit` (database `hub`) for local development. `compose.yaml` provides a self-contained local-production stack (app + postgres) for podman. Treat production database provisioning (Coolify) as separate infrastructure.
